        ...
        {allTypes.map((item) => (
          <Link to={`/${item.id}`} key={item.id}>
            <div
              className={`${
                item.id === type ? '' : ''
              } pb-3 transition duration-700 ease-out flex items-center gap-2`}
            >
              <div className='' style={{ height: '52px', width: '52px' }}>
                <img
                  src={item.id === type ? item.imageActive : item.imageDefault}
                  alt={item.id}
                  className={`${
                    item.id === type ? 'bg-blue-200' : ''
                  } w-full h-full rounded-3xl  p-2 `}
                ></img>
              </div>
              <h1
                className={`${
                  type === item.id ? 'text-zomato-400' : 'text-gray-400'
                }  text-2xl`}
              >
                {item.name}
              </h1>
            ...
            <hr
              className={`${item.id === type ? 'hrTransition' : 'border-0'} `}
            />
          </Link>
        ))}
      </div>
...
function FoodTabMobile() {
  ...
  return (
    <div
      className='flex lg:hidden justify-around z-20 fixed bottom-0 bg-white py-2 foodTab'
      style={{ width: '100vw' }}
    >
      {allTypes.map((item) => (
        <Link to={`/${item.id}`} key={item.id} className='text-center w-16'>
          <hr
            className={`${item.id === type ? 'hrTransition' : 'border-0'} mb-1`}
          />
          <div classNaame='flex flex-col gap-1  items-center'>
            <span
              className={`${type === item.id ? 'text-zomato-400' : ''} text-sm`}
            >
              {item.icon}
            </span>
